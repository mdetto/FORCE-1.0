function [Ab,x,R0,Idn,Iup,A,P] = RT2S_analytical(LAI,I0,ze,fD,rho,varargin)
...
%% albedo calculation
G = @(lambda) ((alfa+gamma+lambda-rhos*gamma)*sbw-(rhos*(alfa+gamma-lambda)-gamma)*sfw)/(k+lambda)*(1-fD) + ...
    (gamma -rhos*(alfa+gamma-lambda))*fD;
Q = (rhos + ((alfa+gamma-k)*sbw+gamma*sfw - rhos*((alfa+gamma+k)*sfw+gamma*sbw))/(k+lambda)/(k-lambda))*2*lambda*(1-fD);
...
A = @(lambda) (((alfa+gamma+lambda)*sfw+gamma*sbw)/(k-lambda)*(1-fD) + (alfa+gamma+lambda)*fD - gamma*rhoc)/(2*lambda);
B = @(lambda) (((alfa+gamma-lambda)*sbw+gamma*sfw)/(k-lambda)*(1-fD) + gamma*fD - (alfa+gamma-lambda)*rhoc)/(2*lambda);
C = @(k,sfw,sbw) -((alfa+gamma+k)*sfw+gamma*sbw)/((k+lambda)*(k-lambda))*(1-fD);
...
if err>1e-6
    disp('error bottom boundary condition')
end
...
P = zeros(length(x),m+1);

wm = 1/(2*m):1/m:(2*m-1)/(2*m);
p = 1/m;

A(:,1) = alfa*(Idn+Iup); %shade
A(:,2:m+1) = alfa*(I0*(1-fD)/cos(ze).*wm+Idn+Iup); %sunlit
gap = exp(-k*x).';
P(:,1) = 1-gap; % shade
P(:,2:m+1) = repmat(p.*gap,1,m); %sunlit
...
% [~,x2,~,Idn,Iup,albedo] = RT2S_SW(L,1,ze,fD);
% 
% hold all
